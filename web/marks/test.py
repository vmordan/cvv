...
from web.utils import CVTestCase, ArchiveFileContent
from web.vars import JOB_STATUS, SAFE_VERDICTS, UNSAFE_VERDICTS, MARK_SAFE, MARK_UNSAFE, MARK_STATUS, MARK_TYPE, \
    PROBLEM_DESC_FILE, ASSOCIATION_TYPE

REPORT_ARCHIVES = os.path.join(settings.BASE_DIR, 'reports', 'test_files')


class TestMarks(CVTestCase):
    def setUp(self):
    ...
    def test_unsafe(self):
        ...
        try:
            compare_f = MarkUnsafeCompare.objects.get(name="model functions")
        except ObjectDoesNotExist:
        ...
        self.assertEqual(mark.version, 1)
        self.assertEqual(mark.description, 'Mark description')
        self.assertEqual(mark.function.name, "model functions")
        self.assertEqual(mark.is_modifiable, True)
        self.assertEqual(len(mark.versions.all()), 1)
