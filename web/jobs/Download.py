...
from service.models import SolvingProgress, JobProgress
...
from web.utils import logger, file_get_or_create, unique_id, BridgeException, OpenFiles
from web.vars import FORMAT, JOB_STATUS, REPORT_ARCHIVE, JOB_WEIGHT
...
class JobArchiveGenerator:
    ...
    def __get_progress_data(self):
        ...
        try:
            ...
            data.update({
                'priority': sp.priority, 'scheduler': 'base',
    ...
    def __create_progress(self, job, data, conf_file):
        if 'scheduler' in data:
            SolvingProgress.objects.create(
                job=job, fake=True, priority=data['priority'],
