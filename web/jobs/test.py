...
from web.vars import JOB_ROLES, JOB_STATUS


class TestJobs(CVTestCase):
    def setUp(self):
    ...
    def test_files(self):
        ...
        response = self.client.post(reverse('jobs:form', args=[job_template.pk, 'copy']), {
            'name': 'New job title 3', 'description': 'Description of new job', 'parent': job_template.identifier,
    ...
    def test_run_decision(self):
        ...
        response = self.client.post(reverse('jobs:form', args=[job_template.id, 'copy']), {
            'name': 'New job title 5', 'description': 'Description of new job',
            'global_role': JOB_ROLES[0][0], 'user_roles': '[]',
