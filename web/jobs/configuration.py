...
from web.vars import PRIORITY, JOB_WEIGHT

# Each CV Core mode represents sets of values for following sets of attributes:
#   scheduling and decision weight:
#     job priority - see web.vars.PRIORITY for available values,
...
class Parallelism:
    ...
    # Each CV Core parallelism pack represents set of numbers of parallel threads/processes
...
class BooleanValues:
    ...
    boolean_names = (
        ('keep_files', 'keep intermediate files',
         _('Keep intermediate files inside the working directory of CV Core')),
        ...
        ('ignore_core', 'ignore other instances', _('Ignore other instances of CV Core')),
...
class GetConfiguration:
    def __init__(self, conf_name=None, file_conf=None, user_conf=None, last_run=None):
        if conf_name is not None:
            ...
            conf_args = self.__get_default_conf_args(settings.DEF_CV_CORE_MODE)
        self.configuration = Configuration(*conf_args)
        self.modes = list((m['id'], m['name']) for m in CV_CORE_DEF_MODES)

    def __get_default_conf_args(self, name):
        for mode in CV_CORE_DEF_MODES:
            if name == mode['id']:
                return mode['values']
    ...
    def __conf_args_from_json(self, file):
        ...
        return [
            [filedata['priority'], filedata['max solving tasks per sub-job'], filedata['weight']],
