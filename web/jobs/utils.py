...
from web.utils import logger, BridgeException, file_get_or_create, get_templated_text
from web.vars import JOB_STATUS, USER_ROLES, JOB_ROLES, JOB_WEIGHT, SAFE_VERDICTS, UNSAFE_VERDICTS, \
...
def get_job_children(user, job):
    ...
    for child in job.children.order_by('-id'):
        if JobAccess(user, child).can_view():
            ...
            if report:
                res = ComponentResource.objects.filter(report__root=report.root, report__parent=None,
